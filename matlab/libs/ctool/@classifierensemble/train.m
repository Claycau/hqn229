...
% function [ model ] = train(model, urbilder, bilder, sampleclass, trainparams, eps, varargin)
%
% Simple straight-forward cross-validation for classifier ensembles
%
% Joerg Wichard 2004
%
  ...
  mean_cv_error = zeros(1,trainparams.nr_cv_partitions);
  ...
  for i=1:trainparams.nr_cv_partitions  
	...
	disp('accuracy    precision   recall    fscore   fp_rate  specificity  ber'); 
   ...
   for j=1:nsel
     ...
     %% Ensemble Gleichgewichtet
     %model.ensemble=addmodel(model.ensemble, thismodel(best_model(j)), 1.0, thiserr{best_model(j)});
   ...
   mean_cv_error(i) = testerrors(best_model(1));
  
  end
  
  disp( '%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%');
  disp(['Classifier-Ensemble-Trained - Mean CV-Error: ', num2str(mean( mean_cv_error )),'  ', trainparams.scoring]);
