...
%
%
%   Train mlp class training algorithm based on the Rprop.
%
...
%% Joerg Wichard 2002
  ...
  for l=1:model.layers 
    ...
    %% Die Schrittweite des Gradientenabstiegs
    dw{l}     = (trainparams.mrate_init)*rand( (1+model.topo(l)), model.topo(l+1) );
    dw_old{l} = (trainparams.mrate_init)*rand( (1+model.topo(l)), model.topo(l+1) );
    dstep{l}  = (trainparams.mrate_init)*rand( (1+model.topo(l)), model.topo(l+1) );
  ...
  for iter=1:trainparams.rounds
    ...
    %% Den Epsilon-Insensitiven-Fehler - eps-margin !!
    ecount = weights_train.*((sign(xreal).*( xreal - x )) > epsi );

    %% Der Fehler am Netzausgang
    %%delta{(model.layers + 1)} = -(ecount).*((xreal - x) - epsi );
    delta{(model.layers + 1)} = -(ecount).*sign(xreal - x).*abs(abs(xreal - x) - epsi );
    ...
    for l=1:model.layers
      dnew{l} = ( (transpose([ y{l}, model.offsett*ones(m,1) ])*delta{l+1} ) + ...
		  model.decay*(model.w{l}./((1+model.w{l}.^2).^2)) );
    end 
